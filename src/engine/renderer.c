...
void render_health_bar(memory* g, float x, float y, float health, float max_health) {
    ...
    if (health_ratio > 0.0f) {
        ...
        render_scaled_sprite(g, TEXTURE_HEALTH_FILL,
                           bar_x + fill_offset, bar_y, fill_width, 32.0f);
    }

    render_scaled_sprite(g, TEXTURE_HEALTH_BAR,
...
void render_entities(memory* g) {
    ...
    for (i = 0; i < scene.entity_count; i++) {
        ...
        render_sprite_pixel_perfect(g, e->sprite_sheet.texture_id, x, y,
