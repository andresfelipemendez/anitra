...
static void sProcessMessage(uSynergyContext *context, const uint8_t *message) {
  ...
  if (memcmp(message + 4, "Synergy", 7) == 0) {
    ...
    // message is
    // always coming from the server? 		1 uint32:	The total size
    // of the remaining 'string' (as per the Synergy %s string format (which is
    // 1 uint32 for size followed by a char buffer (not necessarily null
    // terminated)). 		1 uint32: The number of formats present in the
    // message
